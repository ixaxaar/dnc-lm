...
class DNC(nn.Module):
  ...
  def _layer_forward(self, input, layer, hx=(None, None)):
    ...
    for time in range(max_length):
      ...
      # out, chx = self.rnns[layer](input[time].unsqueeze(1), [x.unsqueeze(0) for x in chx])
      # chx = [x.squeeze(0) for x in chx]
      # out = out.squeeze(1)
      # the interface vector
      ξ = chx[0] if self.mode.lower() == 'lstm' else chx
      # the output
